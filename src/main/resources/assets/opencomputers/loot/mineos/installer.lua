...
------------------------------------- Проверка компьютера на соответствие сис. требованиям -------------------------------------
...
do
  ...
  getFromGitHubSafely(GitHubUserUrl .. "Yaroslavdav2020/OpenComputers/master/MineOS/License/" .. _G.OSSettings.language .. ".txt", "/MineOS/System/OS/License.txt")
...
file:write("local success, reason = pcall(loadfile(\"OS.lua\")); if not success then print(\"Îøèáêà: \" .. tostring(reason)) end")
file:close()
